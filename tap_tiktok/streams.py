...
class AdAccountsStream(TikTokStream):
    ...
    def get_url_params(
        ...
        params: dict = {
            "advertiser_ids": "{advertiser_ids}".format(
                advertiser_ids=json.dumps([str(self.config["advertiser_id"])])
            )
        }
        if next_page_token:
...
class AdsMetricsByDayStream(TikTokReportsStream):
    ...
    def get_url_params(
        ...
        if (
            ...
            and next_page_token["start_date"] is not None
        ):
...
class AdsAttributeMetricsStream(AdsMetricsByDayStream):
    ...
    properties += [
        th.Property(
            metric,
            th.StringType if metric in ["campaign_id", "adgroup_id"] else th.StringType,
        )
...
class AdGroupsBasicDataMetricsByDayStream(AdGroupsMetricsByDayStream):
    name = "adgroups_basic_data_metrics_by_day"
    tiktok_metrics = BASIC_DATA_METRICS
    path = "/"
    primary_keys = ["adgroup_id", "stat_time_day"]
    replication_key = "stat_time_day"
    properties = [
        th.Property("adgroup_id", th.StringType),
        th.Property("adgroup_name", th.StringType),
...
class AdGroupsPageEventMetricsByDayStream(AdGroupsMetricsByDayStream):
    name = "adgroups_page_event_metrics_by_day"
    status_field = "adgroups_status"
    tiktok_metrics = PAGE_EVENT_METRICS
    path = "/"
    primary_keys = ["adgroups_id", "stat_time_day"]
    replication_key = "stat_time_day"
    properties = [
...
class AdsInAppEventMetricsByDayStream(AdsMetricsByDayStream):
    ...
    properties += [
        th.Property(metric, th.StringType) for metric in IN_APP_EVENT_METRICS
    ]
    schema = th.PropertiesList(*properties).to_dict()
    ...
    def request_records(self, context: Optional[dict]) -> Iterable[dict]:
        ...
        while not finished:
            ...
            for i in range(2):
                ...
                self.tiktok_metrics = IN_APP_EVENT_METRICS[
                    chunk_size * i : (i + 1) * chunk_size
                ]
                ...
                for row in self.parse_response(resp):
                    primary_key = tuple(
                        row["dimensions"][key] for key in self.primary_keys
                    )
                    if rows.get(primary_key) is None:
                        rows[primary_key] = row
                    else:
                        rows[primary_key]["metrics"] = {
                            **row["metrics"],
                            **rows[primary_key]["metrics"],
                        }
